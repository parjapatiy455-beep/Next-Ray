...
// Gemini Diagnostics API to help instantly diagnose Gemini Key issues
app.post("/api/voice/diagnose", async (req, res) => {
  try {
    ...
    if (!apiKey) {
      return res.json({
        success: false,
        status: "missing_key",
        message: "No Gemini API key detected. Please define GEMINI_API_KEY as an environment variable, or paste it as a Custom Key under Settings."
      });
    }
    ...
    if (textSuccess && ttsSuccess) {
      ...
      if (!ttsSuccess) {
        ...
        if (ttsError.toLowerCase().includes("quota") || ttsError.toLowerCase().includes("limit")) {
          ...
          debugMessage += "👉 Hint: The Gemini tts model (gemini-3.1-flash-tts-preview) might be restricted on your project or has region restrictions. Ensure you have activated general model access or use a fresh Gemini API key created under a standard Google AI Studio account.\n";
        }
      } else {
